...
from model import CLKG_compgatv3_convE, SupConLoss, relation_contrast, CLKG_compgatv3_DFconv3D
...
class CL(pl.LightningModule):
    def __init__(self, args, model):
        ...
        self.evaluator = Evaluator(num_ent=args.ent_num, batch_size=1024)
...
parser.add_argument("--train_path", type=str, default="./data/FB15K/", help="knowledge graph dataset path")
parser.add_argument("--test_path", type=str, default="./data/FB15K/", help="knowledge graph dataset path")
parser.add_argument("--noise_path", type=str, default=None, help="knowledge graph dataset path")

parser.add_argument("--rel_num", type=int, default=1345*2, help="number of relations in Knowledge Graph")
parser.add_argument("--ent_num", type=int, default=14951, help="number of entites in Knowledge Graph")
...
parser.add_argument("--num_gaussian_kernels", type=int, default=200, help="num_gaussian_kernels")
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,
        verbose=True)
    cl_trainer = pl.Trainer(gpus=1, num_nodes=1, max_epochs=args.cl_epochs, precision=32, callbacks=[checkpoint_callback], check_val_every_n_epoch=2)
    ...
    #dir = './lightning_logs/version_{}/checkpoints'.format(9)
    ...
    if args.checkpoint_path:
        best_path = './lightning_logs/version_{}/checkpoints/{}'.format(9, listdir(dir)[0])
        print(f"Loading pre-trained model from: {args.checkpoint_path}")
        try:
            best_model = CL.load_from_checkpoint(checkpoint_path=best_path, args=args, model=model,
                                                 strict=False)
        ...
        best_model = cl_model.load_from_checkpoint(checkpoint_path=best_path, args=args, model=model, strict=False)
